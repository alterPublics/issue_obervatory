...
- [x] Zeeschuimer-to-4CAT protocol specification (`/docs/research_reports/zeeschuimer_4cat_protocol.md`) -- 2026-02-21
...
- [x] Co-mention detection in snowball sampling (`/docs/research_reports/comention_snowball_recommendation.md`) -- 2026-02-20
- [x] Network expansion API assessment (`/docs/research_reports/network_expansion_api_assessment.md`) -- 2026-02-20
...
- [x] P0-2: Coherency audit triage (`/docs/pre_production_checklist_p0_2_triage.md`) -- 2026-02-21. Triaged 13 critical + 21 moderate findings from both audits. Result: 10 critical fixed, 12 moderate fixed, 4 beta-blocking items remain (enrichment UI button, volume spike display, SMTP warning, retention service test).

## Notes
- 2026-02-21: Completed P0-2 pre-production coherency audit triage. Of 13 critical findings across both audits, 10 have been fixed since the audits were written. 4 beta-blocking items identified: (BB-1) enrichment pipeline has no UI trigger button despite endpoint existing, (BB-2) volume spike alerts are detected and stored but never displayed in any template, (BB-3) SMTP disabled state is invisible to administrators, (BB-4) GDPR retention service has zero test coverage. Estimated fix effort: 1-2 days total across Core, Frontend, and QA agents. Full report at `/docs/pre_production_checklist_p0_2_triage.md`.
- 2026-02-21: Completed Zeeschuimer-to-4CAT protocol specification (`/docs/research_reports/zeeschuimer_4cat_protocol.md`). Full reverse-engineering of the upload protocol from source code analysis of both Zeeschuimer (v1.13.6, JavaScript) and 4CAT (Python/Flask). Key findings: (1) Upload is a single POST to `/api/import-dataset/` with raw NDJSON body (not multipart) and `X-Zeeschuimer-Platform` header; (2) Each NDJSON line is a Zeeschuimer envelope with `data` nested object containing raw platform JSON; 4CAT restructures this into `__import_meta` + flattened data; (3) Authentication via browser session cookies or `access-token` query parameter/header; (4) LinkedIn data uses Voyager V2 format with imprecise relative timestamps (no absolute timestamps from LinkedIn); (5) Complete platform module_id to 4CAT datasource name mapping documented for all 14 modules; (6) Detailed field mapping from LinkedIn Voyager V2 to IO content_records schema; (7) Recommended implementation as import endpoint (not arena collector), estimated 5.5 person-days; (8) Key gotcha: Zeeschuimer strips configured URL to protocol+domain only, so query parameter auth tokens are lost -- cookie-based auth is the practical approach.
- 2026-02-20: Completed network expansion API assessment (`/docs/research_reports/network_expansion_api_assessment.md`). Evaluated 8 platforms for follower/following graph traversal capabilities. Key findings: (1) TikTok Research API has dedicated followers/following endpoints with 20,000 calls/day quota (separate from video query quota) -- this is the highest-priority implementation target at zero cost; (2) Gab's Mastodon-compatible API supports standard `/followers` and `/following` endpoints at zero cost but needs live verification against Gab's fork; (3) TwitterAPI.io provides follower/following endpoints at $0.15/1K users -- modest cost for high research value; (4) Discord member lists are not graph data; (5) Facebook and Threads have no follower list APIs available at our tiers; (6) Instagram is technically feasible via Bright Data but cost-prohibitive for routine snowball sampling; (7) Majestic's backlink graph operates at domain level, not actor level. Recommended 3-phase implementation: TikTok (0.5-1 day), Gab (0.5 day), X/Twitter (1 day). After implementation, 4 of 11 social media platforms will have structural graph expansion. Arena briefs for Gab and X/Twitter should be updated to document follower/following endpoints.
- 2026-02-20: Completed methodology recommendation on co-mention detection in snowball sampling. Report at `/docs/research_reports/comention_snowball_recommendation.md`. Key findings: (1) co-mention detection is a methodologically distinct strategy (discursive association vs. structural graph traversal) that is currently hidden from researchers as a silent fallback; (2) the `discovery_method` field is populated by `NetworkExpander` and preserved by `SnowballSampler` but silently dropped by the `SnowballActorEntry` response schema in the API route -- this is a critical transparency gap; (3) four recommendations: surface discovery method in results (critical, 2-3 hours), allow explicit co-mention platform selection (high, 1-2 days), expose standalone `find_co_mentioned_actors()` as a discovery tool (high, 2-3 days), add evidence strength indicators (medium, 0.5-1 day); (4) no database migrations required for any recommendation. The standalone `find_co_mentioned_actors()` method (lines 243-323 of `network_expander.py`) is a fully implemented but unexposed feature that answers "which actors tend to appear together in my collected data?" -- distinct from the per-actor `_expand_via_comention()` used during snowball expansion.
- 2026-02-19: Created `/CLAUDE.md` and updated `/clinerules` to reflect actual implementation state. Changes: (1) CLAUDE.md created as comprehensive project guide covering all 24 arena directories (21 functional), 9 database migrations, enrichment pipeline, analysis module, scraper module, sampling module, frontend pages, API routes, and IP2/GR implementation progress; (2) clinerules updated to reflect: arena registry keyed by platform_name not arena_name, ArenaCollector interface with term_groups/language_filter/public_figure_ids, researcher-configurable source lists (GR-01 through GR-04), enrichment pipeline pattern, content_records partitioning, respx for HTTP mocking, updated file ownership (scraper module, frontend templates/static), updated references to include implementation_plan_2_0_strategy.md and status files; (3) removed outdated guidance: `web_at_large` arena name example replaced with actual names, `content_items` references corrected to `content_records`, testing coverage target updated to 75% for arenas (matching actual QA status), added `scraper` scope for commit messages.
