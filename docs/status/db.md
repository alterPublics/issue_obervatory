...
- [x] F-05: Stop word filtering in emergent term extraction — Added comprehensive Danish and English stop word lists (150+ words) to `get_emergent_terms()` TF-IDF feature; filters stop words BEFORE TF-IDF computation and applies post-filtering for single-character and numeric tokens; eliminates common function words ("og", "the", "at", "er", etc.) from suggested terms results — FIXED (2026-02-23)
