...
def ritzau_via_collect_terms(
    ...
    count = len(records)
    logger.info(
        "ritzau_via: collect_by_terms completed — run=%s records=%d",
        collection_run_id,
        count,
    )
    _update_task_status(collection_run_id, "ritzau_via", "completed", records_collected=count)
    publish_task_update(
        redis_url=_redis_url,
        run_id=collection_run_id,
        arena="news_media",
...
def ritzau_via_collect_actors(
    ...
    count = len(records)
    logger.info(
        "ritzau_via: collect_by_actors completed — run=%s records=%d",
        collection_run_id,
        count,
    )
    _update_task_status(
        collection_run_id, "ritzau_via", "completed", records_collected=count
    )
    publish_task_update(
        redis_url=_redis_url,
        run_id=collection_run_id,
