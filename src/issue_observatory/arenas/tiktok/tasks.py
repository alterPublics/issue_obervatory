...
def tiktok_collect_terms(
    ...
    **_extra: Any,
    ...
    if not force_recollect and date_from and date_to:
        ...
        if not gaps:
            logger.info(
                "tiktok: full coverage exists for run=%s — skipping API call, "
            ...
            _update_task_status(
                collection_run_id, "tiktok", "completed", records_collected=0
            )
            publish_task_update(
                redis_url=_redis_url,
                run_id=collection_run_id,
        ...
        logger.info(
            "tiktok: narrowing collection to uncovered range %s — %s (run=%s)",
            effective_date_from,
            effective_date_to,
            collection_run_id,
        )
    ...
    if inserted == 0:
        ...
        db_count = count_run_platform_records(collection_run_id, "tiktok")
        if db_count > 0:
            logger.info("tiktok: in-memory counter=0 but DB has %d records — using DB count", db_count)
            inserted = db_count

    # Record successful collection attempts for future pre-checks.
    # Use per-term counts from the collector so that terms that genuinely
    # returned 0 results are recorded accurately, while the total run count
    # is not broadcast to every term.
    ...
    logger.info(
        "tiktok: collect_by_terms completed — run=%s emitted=%d inserted=%d skipped=%d",
        collection_run_id,
        collector.batch_stats["emitted"],
        inserted,
        skipped,
    )
    _update_task_status(collection_run_id, "tiktok", "completed", records_collected=inserted)
    publish_task_update(
        redis_url=_redis_url,
        run_id=collection_run_id,
        arena="social_media",
...
def tiktok_collect_actors(
    ...
    **_extra: Any,
    ...
    logger.info(
        "tiktok: collect_by_actors completed — run=%s emitted=%d inserted=%d skipped=%d",
        collection_run_id,
        collector.batch_stats["emitted"],
        inserted,
        skipped,
...
@celery_app.task(
    name="issue_observatory.arenas.tiktok.tasks.collect_comments",
    bind=True,
    max_retries=3,
    autoretry_for=(ArenaRateLimitError,),
    retry_backoff=True,
...
def tiktok_collect_comments(
    ...
    **_extra: Any,
    ...
    """Collect comments for a list of TikTok videos.

    Calls :meth:`TikTokCollector.collect_comments` for the given video IDs,
    persists the resulting records to the database, and emits SSE progress
    updates on the collection run's event stream.

    Args:
        query_design_id: UUID string of the owning query design.
        collection_run_id: UUID string of the owning collection run.
        post_ids: List of dicts, each with a ``platform_id`` key containing a
            TikTok video ID string.
        tier: Tier string — always ``"free"`` for TikTok in Phase 1.
        max_comments_per_post: Maximum comments to collect per video.
        depth: Passed through to the collector but unused (TikTok returns a
            flat comment list regardless of nesting depth).
    ...
    logger.info(
        "tiktok: collect_comments started — run=%s post_ids=%d",
        collection_run_id,
        len(post_ids),
    )
    _update_task_status(collection_run_id, "tiktok_comments", "running")
    publish_task_update(
        redis_url=_redis_url,
        run_id=collection_run_id,
        arena="tiktok_comments",
        platform="tiktok",
        status="running",
        records_collected=0,
        error_message=None,
    ...
    try:
        ...
        msg = f"tiktok: no credential available for collect_comments: {exc}"
        logger.error(msg)
        _update_task_status(
            collection_run_id, "tiktok_comments", "failed", error_message=msg
        )
        publish_task_update(
            redis_url=_redis_url,
            run_id=collection_run_id,
        ...
        raise ArenaCollectionError(
            msg, arena="tiktok_comments", platform="tiktok"
        ) from exc
    except ArenaRateLimitError:
        logger.warning(
            "tiktok: rate limited on collect_comments for run=%s — will retry.",
            collection_run_id,
        )
        raise
    except (ArenaAuthError, ArenaCollectionError) as exc:
        msg = str(exc)
        logger.error(
            "tiktok: comment collection error for run=%s: %s", collection_run_id, msg
        )
        _update_task_status(
            collection_run_id, "tiktok_comments", "failed", error_message=msg
        )
    ...
    _update_task_status(
        collection_run_id, "tiktok_comments", "completed", records_collected=inserted
    )
    publish_task_update(
        redis_url=_redis_url,
        run_id=collection_run_id,
