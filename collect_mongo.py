...
import spreadAnalysis.utils.helper as hlp
...
class CollectMongo:
    ...
    def resolve_dates_from_pulls(self,input,method,input_type,start_date,end_date,recollect=False,skip_existing=False):
        ...
        if isinstance(input,list):
            for inp in input:
                prev_pulls = list(self.mdb.database["pull"].find({"input":inp,"method":method,"input_type":input_type}))
                if len(list(prev_pulls)) > 0:
                    attempts.extend(list(prev_pulls[0]["attempts"]))
        else:
            prev_pulls = list(self.mdb.database["pull"].find({"input":input,"method":method,"input_type":input_type}))
            if len(list(prev_pulls)) > 0:
    ...
    def process_pull(self,input,method,input_type,data,start_date,end_date,extra_fields={}):
        ...
        if self.mdb.database["pull"].count_documents({ "input": input ,"method": method,"input_type":input_type }, limit = 1) != 0:
    ...
    def get_clean_url(self,org_url,is_domain=False,with_unpack=True):
        ...
        if not is_domain:
            ...
            if not isinstance(clean_url,list) and clean_url is not None:
    ...
    def get_methods(self,endpoint,custom_platforms=None):

        methods = {}
        for platform in self.platform_info:
            if custom_platforms is not None and platform["platform_dest"] not in set([cp["platform_dest"] for cp in custom_platforms]): continue
            if platform["working"]:
                if platform[endpoint+"_endpoint"]:
                    if platform["platform_source"] not in methods or platform["platform_dest"] == "Instagram":
                        ...
                        elif platform["platform_dest"] in set(["Reddit"]):
                            method = self.collecters[platform["platform_source"]]()
    ...
    def url_collect(self,org_urls,input_sd,input_ed,recollect=False,with_unpack=True,reclean=True):
        ...
        for org_url in org_urls:
            ...
            if cleaned_url is None:
                if reclean:
                    cleaned_url = self.get_clean_url("https://"+org_url,with_unpack=with_unpack)
                    cleaned_urls[org_url]=cleaned_url
                    if cleaned_url is None:
                        continue
                else:
            ...
            for method_name, method in self.get_methods("url").items():
                ...
                start_date, end_date = self.resolve_dates_from_pulls(resolve_inputs,method_name,"url",start_date,end_date,recollect=recollect)
    ...
    def actor_collect(self,org_actors,input_sd,input_ed,custom_aliases=None,recollect=False):

        if custom_aliases is None:
            aliases = self.mdb.get_aliases(actors=org_actors)
        else:
        ...
        actor_aliases = self.mdb.get_actor_aliases(actors=org_actors)
            ...
            if actor in aliases:
                for alias in aliases[actor]:
                    if alias["platform"] in platform_to_source:
                        ...
                        if method_name == "tiktok2":
                            sd_ed_ranges = hlp.create_date_ranges(input_sd,input_ed,interval=29)
                        else:
                            sd_ed_ranges = [(input_sd,input_ed)]
                        for start_date, end_date in sd_ed_ranges:
                            start_date, end_date = self.resolve_dates_from_pulls(resolve_inputs,method_name,"actor",start_date,end_date,recollect=recollect)
                            if start_date is None:
                                continue
                            #if True:
                            try:
                                data = method.actor_content(platform_alias,start_date=start_date,end_date=end_date)
                                if isinstance(data,str) and data == "SKIP":
                                    break
                                else:
    ...
    def domain_collect(self,org_urls,input_sd,input_ed,actor_web=False,recollect=False):
        ...
        for org_url in org_urls:
            ...
            for method_name, method in self.get_methods("domain").items():
                start_date, end_date = input_sd, input_ed
                start_date, end_date = self.resolve_dates_from_pulls(resolve_inputs,method_name,"domain",start_date,end_date,recollect=recollect)
                if start_date is None:
                    continue
    ...
    def query_collect(self,org_queries,input_sd,input_ed,recollect=False):
        ...
        for org_q in org_queries:
            ...
            for method_name, method in self.get_methods("query").items():
                start_date, end_date = input_sd, input_ed
                start_date, end_date = self.resolve_dates_from_pulls(resolve_inputs,method_name,"query",start_date,end_date,recollect=recollect)
                if start_date is None:
                    continue
                l_start_date, l_end_date = start_date, start_date
                current_interval = self.MIN_DATE_RANGE_INTERVAL
                while hlp.to_default_date_format(l_end_date) < hlp.to_default_date_format(end_date):
                    ...
                    if self.CIRCLE_REGION:
                        region_codes = self.TIKTOK_REGION_CODES
                    #if True:
                    data = []
                    for region_code in region_codes:
                        print (region_code)
                        try:
                            data.extend(list(method.query_content(org_q,start_date=l_start_date,end_date=l_end_date,region_code=region_code)["output"]))
                        except:
    ...
    def actor_info_collect(self,org_actors,recollect=False,custom_aliases=None):
        ...
        if custom_aliases is None:
            ...
            aliases = custom_aliases
        actor_aliases = self.mdb.get_actor_aliases(actors=org_actors)
        methods = self.get_methods("actor_info")
        if "tiktok2" in methods: methods["tiktok2"]._check_time()
        platform_to_source = {d["platform_dest"]:d["platform_source"] for d in self.platform_info}
                        ...
                        if alias["platform"] in actor_aliases:
                            if platform_alias in actor_aliases[alias["platform"]]:
                                for actor_alias in actor_aliases[alias["platform"]][platform_alias]:
                                    resolve_inputs.append(actor_alias)
                        resolve_inputs = list(set(resolve_inputs))
                        start_date, end_date = self.resolve_dates_from_pulls(resolve_inputs,method_name,"actor_info","1989-03-10","1989-03-10",recollect=recollect)
                        if start_date is None:
                            continue
                        #if True:
                        try:
                            data = method.get_user_info(platform_alias)
                            data["actor"]=actor
                            data["actor_platform"]=actor+"_Tiktok"
                        except Exception as e:
                            data = None
                            print (e)
                        self.process_pull(actor,method_name,"actor_info",[data],start_date,end_date)
                        if data is not None:                            
                            prev_post_ids = self.save_data(self.mdb.database["actor_info"],
                                [data],method_name,None,"actor_platform",update_key_col="actor_platform")
                            print (str(platform_alias) + " - " + str(method_name) + " - " + str(len(data)))
                        else:
                            print (str(platform_alias) + " - " + str(method_name) + " - " + str("ERROR in Data Retrieval"))

    def followers_collect(self,org_actors,direction="followers",custom_aliases=None,recollect=False):
        ...
        if custom_aliases is None:
            ...
            aliases = custom_aliases
        actor_aliases = self.mdb.get_actor_aliases(actors=org_actors)
        methods = self.get_methods(direction)
        if "tiktok2" in methods: methods["tiktok2"]._check_time()
        platform_to_source = {d["platform_dest"]:d["platform_source"] for d in self.platform_info}
                        ...
                        if alias["platform"] in actor_aliases:
                            if platform_alias in actor_aliases[alias["platform"]]:
                                for actor_alias in actor_aliases[alias["platform"]][platform_alias]:
                                    resolve_inputs.append(actor_alias)
                        resolve_inputs = list(set(resolve_inputs))
                        start_date, end_date = self.resolve_dates_from_pulls(resolve_inputs,method_name,direction,"1989-03-10","1989-03-10",recollect=recollect)
                        if start_date is None:
                            continue
                        #if True:
                        try:
                            if direction == "followers":
                                data = method.followers(platform_alias)
                            elif direction == "following":
                                data = method.following(platform_alias)
                            if isinstance(data,str) and data == "SKIP":
                                break
                            else:
                                data = data["output"]
                            for doc in data:
                                doc["actor"]=actor
                                doc["actor_username"]=platform_alias
                                doc["actor_platform"]=actor+"_Tiktok"
                                doc["username_actor"]=doc["username"]+platform_alias
                        except Exception as e:
                            data = None
                            print (e)
                        self.process_pull(actor,method_name,direction,data,start_date,end_date)
                        if data is not None:
                            if direction == "followers":
                                save_to = "follower"
                            elif direction == "following":
                                save_to = "following"                            
                            prev_post_ids = self.save_data(self.mdb.database[save_to],
                                data,method_name,None,"username_actor",update_key_col="username_actor")
                            print (str(platform_alias) + " - " + str(method_name) + " - " + str(len(data)))
                        else:
                            print (str(platform_alias) + " - " + str(method_name) + " - " + str("ERROR in Data Retrieval"))

    def comments_collect(self,posts,platform,recollect=False):

        method = self.get_methods("comments",custom_platforms=[platform])[platform["platform_source"]]
        method_name = platform["platform_source"]
        for post in posts:
            resolve_inputs = list([post])
            start_date, end_date = self.resolve_dates_from_pulls(resolve_inputs,method_name,"comments","1989-03-10","1989-03-10",recollect=recollect)
            if start_date is None:
                continue
            try:
                data = method.comments(post.replace(method_name+"_",""))["output"]
            except Exception as e:
                data = None
                print (e)
            self.process_pull(post,method_name,"comments",data,start_date,end_date)
            if data is not None:
                prev_post_ids = self.save_data(self.mdb.database["comment"],
                    data,method_name,None,"comment_id",update_key_col="comment_id")
                print (str(post) + " - " + str(method_name) + " - " + str(len(data)))
            else:
                print (str(post) + " - " + str(method_name) + " - " + str("ERROR in Data Retrieval"))
    ...
    def collect(self,endpoint,title=None,platform_list=None,start_date=None \
                ...
                custom_org_inputs=[],recollect=False,assume_unique_posts=True):
        
        if isinstance(endpoint,tuple):
            call_endpoint = endpoint[1]
            endpoint = endpoint[0]
        else:
            if endpoint == "domain": call_endpoint = "url"
            elif endpoint == "actor_info": call_endpoint = "actor"
            elif endpoint == "followers": call_endpoint = "actor"
            elif endpoint == "following": call_endpoint = "actor"
        ...
        if endpoint == "actor_info":
            org_inputs = [actor_doc["Actor"] for actor_doc in org_inputs]
            if len(custom_org_inputs) > 0:
                org_inputs = [i for i in org_inputs if i in custom_org_inputs]
            self.actor_info_collect(org_inputs,recollect=recollect)
        if endpoint == "followers" or endpoint == "following":
            org_inputs = [actor_doc["Actor"] for actor_doc in org_inputs]
            if len(custom_org_inputs) > 0:
                org_inputs = [i for i in org_inputs if i in custom_org_inputs]
            self.followers_collect(org_inputs,recollect=recollect,direction=endpoint)
        if endpoint == "domain":
            org_inputs = set([d["Url"] for d in list(org_inputs) if str(d["Domain"]) == "1" or str(d["Domain"]) == "1.0"])
            if actor_web:
                if len(iterations) > 0 and title is not None:
                    org_inputs.update(set([d["Website"].lower() for d in \
                        self.mdb.database["actor"].find({ "Iteration": { "$in": iterations },  "org_project_title":{"$eq":title}}) if len(str(d["Website"])) > 0 and d["Website"] is not None]))
                else:
                    org_inputs.update(set([d["Website"].lower() for d in \
                        self.mdb.get_data_from_db(self.mdb.database["actor"]) if len(str(d["Website"])) > 0 and d["Website"] is not None]))
        ...
        if endpoint == "comments":
            if call_endpoint == "actor":
                org_inputs = [actor_doc["Actor"] for actor_doc in org_inputs]
                if len(custom_org_inputs) > 0:
                    org_inputs = [i for i in org_inputs if i in custom_org_inputs]
                for platform in self.platform_info:
                    if assume_unique_posts:
                        post_ids = {d["message_id"] for d in self.mdb.database["actor_post"].find({"input":{"$in":org_inputs}})}
                        self.comments_collect(post_ids,platform,recollect=recollect)
            if call_endpoint == "query":
                if len(custom_org_inputs) == 0:
                    print ("custom inputs needed")
                    sys.exit()
                org_inputs = custom_org_inputs
                for platform in self.platform_info:
                    post_ids = {d["message_id"] for d in self.mdb.database["query_post"].find({"input":{"$in":org_inputs}})}
                    post_ids = list([self.mdb.database["post"].find_one({"message_id":mid}) for mid in post_ids])
                    post_ids = list([d["message_id"] for d in post_ids if "comment_count" in d and d["comment_count"]>100])
                    self.comments_collect(post_ids,platform,recollect=recollect)
